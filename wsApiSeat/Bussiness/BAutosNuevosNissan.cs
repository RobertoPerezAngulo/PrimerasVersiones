...
namespace wsApiSeat.Bussiness
{
    ...
    public class BAutosNuevosNissan
    {
        ...
        internal static Respuesta PostMeInteresa(MeIntersa obj)
        {
            ...
            //string rutaArchivo = obtenerURLServidor();
            try
            {
                ...
                if (response.Ok.Equals("SI"))
                {
                    ...
                    if (dtdn.Rows.Count == 1)
                    {
                        ...
                        strSql = "";
                        strSql = @"SELECT * FROM " + constantes.Ambiente + "APPS.APDCNCST WHERE FIAPIDCIAU = " + obj.DatosCliente.IdAgencia + " AND  FSAPTIPAUT = " + (int)ETipoAuto.NUEVO;
        ...
        internal static Respuesta PostRegistraCuenta(Cuenta cuenta, DVADB.DB2 dbCnx, DVAConstants.Constants constantes)
        {
            ...
            try
            {
                ...
                long idCuenta = 0;
                string strSql = "";
                //strSql = "SELECT *  FROM PRODAPPS.APCCTAST WHERE lower(TRIM(FSAPCORREO)) = '" + cuenta.Correo.Trim().ToLower() + "'";
                ...
                respuesta.Ok = "SI";
                respuesta.Mensaje = "El auto de interés se guardó correctamente.";
